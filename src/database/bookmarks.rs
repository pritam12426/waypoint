...
//! app. The CLI and HTTP layers share it, so a behavior fixed here (a
//! filter, a search rule, a duplicate message) is identical across both
//! front doors.
//!
//! # Design notes
//!
...
/// names the owner, so CLI and HTTP layers see the same message the
/// pre-check would have produced. (If the row vanished again in the race,
/// fall back to a generic phrasing rather than a raw constraint string.)
...
pub fn insert(conn: &Connection, new: &NewBookmark) -> Result<i64> {
	...
	// `core::media`, shared with the HTTP layer. The CLI calls straight
	// through here; the HTTP handler resolves first — on a separate blocking
	// task, without holding the writer — and calls `insert_resolved`.
...
/// behind `/keywords/{keyword}` and the CLI `open`/`keywords` commands.
///
...
/// All active bookmarks, unpaginated — used by export, not by the API/CLI
/// `list` command (which goes through `list` and its default page size).
...
/// used by the CLI `keywords` command and the `/keywords` route.
///
...
///
/// The CLI calls straight through here: read → collision pre-check →
/// resolve media (`core::media::resolve_update`) → persist. The HTTP
/// handler runs the same three pieces but resolves media *first* (on a
/// separate blocking task, without holding the writer) and then calls
/// `update_resolved`.
...
/// Soft path of `waypoint remove`: move a bookmark into the trash. Sets
/// `trashed_at`; the FTS trash trigger removes its search entry.
/// Returns `false` when the bookmark doesn't exist or is already trashed.
pub fn trash(conn: &Connection, id: i64) -> Result<bool> {
	let changed = conn.execute(
		"UPDATE bookmarks SET trashed_at = CURRENT_TIMESTAMP WHERE id = ?1 AND trashed_at IS NULL",
		params![id],
	)?;
	let ok = changed > 0;
...
/// `--purge` path of `waypoint remove`). The FTS delete trigger cleans up
/// the matching FTS row automatically (the `WHEN OLD.trashed_at IS NULL`
/// guard means purging trash never double-`delete`s); ON DELETE CASCADE
...
///
/// A filter that matches nothing is a harmless no-op, not an error, so
/// `waypoint remove --tag X` on a tag with no bookmarks still succeeds.
///
/// The transaction uses `Connection::unchecked_transaction` (the `&self`
/// variant) rather than `transaction(&mut self)` so the CLI layer — which
/// only holds `&Connection` — can call this. That's safe here because the
/// connection is never shared concurrently: the HTTP layer serializes it
/// behind a `Mutex`, and the CLI is single-threaded.
pub fn remove_matching(
	...
	{
		let mut stmt = if purge {
			tx.prepare("DELETE FROM bookmarks WHERE id = ?1")?
		} else {
			tx.prepare(
...
pub fn remove_ids(conn: &Connection, ids: &[i64], purge: bool) -> Result<BulkRemoveResult> {
	...
	let sql = if purge {
		"DELETE FROM bookmarks WHERE id = ?1 AND trashed_at IS NULL"
	} else {
		"UPDATE bookmarks SET trashed_at = CURRENT_TIMESTAMP
		 WHERE id = ?1 AND trashed_at IS NULL"
	};
...
/// Finds groups of active bookmarks that share the same URL. Each group
/// returns the oldest bookmark id (to keep) and the rest (to remove).
///
/// Two queries: first the groups (`GROUP BY url HAVING COUNT(*) > 1`,
/// most-duplicated first), then one membership query per group for the
/// remove-ids. Trashed bookmarks never participate in a group.
pub fn find_duplicates(conn: &Connection) -> Result<Vec<crate::model::DuplicateGroup>> {
	...
	for (url, keep_id) in urls {
		let remove_ids: Vec<i64> = {
			let mut stmt = conn.prepare(
				"SELECT id FROM bookmarks
				 WHERE url = ?1 AND trashed_at IS NULL AND id != ?2
				 ORDER BY id",
			)?;
			let rows = stmt.query_map(params![url, keep_id], |row| row.get::<_, i64>(0))?;
			rows.collect::<rusqlite::Result<_>>()?
		};
		if !remove_ids.is_empty() {
			groups.push(crate::model::DuplicateGroup {
				url,
				keep_id,
				remove_ids,
			});
	...
	crate::log_trace!("found {} duplicate URL groups", groups.len());
	Ok(groups)
}
...
/// Fetches bookmarks by id, active only, in id order. Used by `dedup` and
/// the API's bulk read paths.
///
