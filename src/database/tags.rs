...
//! SQL statement that touches those two tables — the HTTP layer calls
//! these functions instead of writing its own queries, so tag behavior is
...
/// list (`/api/tags`); `Some` pages it (overview's top 5, `/api/stats/tags`).
