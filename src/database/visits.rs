...
//! or the CLI `open`), recorded by `record`. This module owns that write
//! plus every read that's about *usage* rather than the bookmarks
//! themselves: per-domain counts, most/never-visited bookmarks, and the
//! domain-by-visits ranking.
//!
