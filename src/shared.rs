...
//! Cross-layer helpers shared by the CLI and HTTP layers (and the layers
//! beneath them). This is the dedup seam: anything two layers would
//! otherwise re-implement lives here instead.
//!
//! # What lives here
//!
//! * **Validation** — `validate_limit` / `validate_offset` / `validate_id`
//!   are the *single* place page sizes and ids get checked. Both `cmd` and
//!   `http` produce the same errors from the same rules, so a request that
//!   works over the CLI behaves identically over the API.
...
//! `core`) means the CLI, the HTTP layer, and the persistence layer can all
//! agree on what a "domain" is without a shared dependency between them.
...
/// Validates a bookmark id coming from a URL path or CLI. Bookmarks are
...
/// `String`. Handy for CLI/HTTP layers that collect a batch of optional
