...
/// avatar, a `Fetch`-mode scrape) must never hold the write mutex. The CLI
/// and HTTP share these `resolve_*` functions, so the precedence logic
...
///   2. an explicit payload value (CLI `--favicon`/`--thumbnail`, the
///      HTTP body, an import) still wins over the derivation;
///   3. the empty string is the "no custom media" sentinel (CLI
///      `--no-custom-favicon` / `--no-thumbnail`): favicon falls back to
///      the *generic* domain `favicon.ico` (skipping site-specific rules),
///      thumbnail stays `None`;
