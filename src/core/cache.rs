...
//! `cache_dir` is `$WAYPOINT_CACHE_DIR` if set, else the platform cache
...
/// `$WAYPOINT_CACHE_DIR` wins, then the platform cache directory, then the
/// temp dir.
pub fn cache_dir() -> PathBuf {
	if cfg!(test) {
		...
		FALLBACK
			...
			.clone()
	} else if let Some(dir) = std::env::var_os("WAYPOINT_CACHE_DIR") {
		PathBuf::from(dir)
	} else {
		dirs::cache_dir()
