...
//!
//! Every layer (database, core, cmd, http, and the JSON the frontend
//! consumes) trades these structs around. Keeping them here, free of SQL
//! and I/O, means:
//!
//! * the same `Bookmark` struct that a CLI table printer formats is the
//!   same one serde serializes over HTTP — shapes can't drift;
...
/// explicitly as a URL. Deliberately a tiny enum so the CLI, HTTP, and
/// database layers all speak the same vocabulary:
///
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum, Serialize, Deserialize, ToSchema)]
...
impl std::fmt::Display for AssetMode {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		// Mirrors the serde rename so log lines read like the CLI/API
...
/// A bookmark as stored and returned to clients (CLI JSON output, HTTP API).
///
...
/// A tag's name plus how many bookmarks use it. Used by `/api/tags` and
/// the CLI `tags list`.
#[derive(Debug, Clone, PartialEq, Serialize, ToSchema)]
pub struct TagCount {
	pub name: String,
...
pub struct BookmarkFilter {
	...
	/// CLI never sets it; the HTTP layer decodes it from an opaque
	/// `X-Next-Cursor` token.
