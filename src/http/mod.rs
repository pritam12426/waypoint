...
/// flags failures the handlers didn't already log.
///
...
/// records a `trace` line for every request (method, path, status,
/// elapsed) and picks up the *other* ways a request goes unfulfilled —
...
/// nothing is reported twice. 4xx -> warn, 5xx -> error; both are visible
/// at the default `info` serve log level.
async fn log_request(req: Request, next: Next) -> Response {
	...
	crate::log_trace!("{method} {path}: {status} in {elapsed:?}");
